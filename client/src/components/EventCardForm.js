import { Checkbox, Select, TextField, Typography, FormControlLabel, MenuItem, InputLabel, Menu, Button } from "@mui/material";
    ...
    return (
        <div class="form-wrapper">
            <div class="form">
                ...
                <TextField label="Название события" variant="outlined" onChange={(e) => setEventName(e.target.value)} />

                <TextField label="Локация события" variant="outlined" onChange={(e) => setEventLocation(e.target.value)} />

                <FormControlLabel label="Планируемое событие или внеплановое?" control={
                        <Checkbox defaultChecked onChange={(e) => setIsPlanned(e.target.checked)} />
                } />
                
                <TextField label="Внешняя ссылка" variant="outlined" onChange={(e) => setExternalLink(e.target.value)} />

                <TextField label="Внутренняя ссылка" variant="outlined" onChange={(e) => setInternalLink(e.target.value)} />

                <Typography variant="h5" sx={{ marginTop: '20px'}} > Выберите вид события </Typography>
                <Select variant="outlined" onChange={(e) => setEventKindId(e.target.value)} >
                ...
                <Typography variant="h5" sx={{ marginTop: '20px'}} > Выберите тип события </Typography>
                ...
                <TextField label="Комментарии" variant="outlined" multiline rows={4} onChange={(e) => setComment(e.target.value)} />

                <Typography variant="h5" sx={{ marginTop: '20px'}} > Выберите участников </Typography>
                <StudentPicker students={studentList} setSelectedStudentsList={setSelectedStudentsList} />

                <Typography variant="h5" sx={{ marginTop: '20px'}} > Выберите преподавателя </Typography>
                ...
                <Button variant="filled" onClick={submit_form} sx={{ marginTop: '20px' }}> Создать </Button>
            </div>
        </div>
    )
}
...
function StudentPicker ({ students, setSelectedStudentsList }) {
    ...
    const get_selected_text = (selectedStudents) => {
        ...
        if (selectedStudents.length === 0) {
            ...
            return get_io(get_student_by_id(st1)) + ", " +
                    get_io(get_student_by_id(st2)) + " и еще " + 
                    (selectedStudents.length - 2);
        } 
        else {
            return selectedStudents.map(
                (st_id) => get_io(get_student_by_id(st_id))
            ).join(", ");
        }
    }
    ...
    return (
        <MenuItem>
            <StudentPickerItemDiv onClick={handleChoose}>
                { checked ? 
                    <CheckBoxOutlinedIcon /> : <CheckBoxOutlineBlankOutlinedIcon />
                } 
